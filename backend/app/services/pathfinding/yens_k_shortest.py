...
class YensKShortestPathfinder(PathfindingAlgorithm):
    ...
    async def find_path(
        ...
        k_paths = await self._find_k_shortest_paths(
            graph, start, end, weight, websocket
        )
    ...
    async def _find_k_shortest_paths(
        self, graph: nx.DiGraph, start: str, end: str, weight: str, websocket
    ) -> List[tuple]:
        """Find K shortest paths using Yen's algorithm."""
        # Get shortest path first (using Dijkstra)
        shortest = await self._dijkstra(graph, start, end, weight)
        ...
        for k_iter in range(1, self.k):
            ...
            for i in range(len(k_shortest[-1][0]) - 1):
                ...
                spur_path = await self._dijkstra_with_excluded_edges(
                    graph, spur_node, end, weight, removed_edges
                )

                if spur_path is not None:
                    full_path = root_path[:-1] + spur_path[0]
                    full_cost = self._calculate_path_cost(
                        graph, full_path, weight
                    )
                    candidate = (full_path, full_cost)

                    # Avoid duplicates
                    if candidate not in candidate_paths and candidate not in k_shortest:
                        heapq.heappush(
    ...
    async def _dijkstra(
        self, graph: nx.DiGraph, start: str, end: str, weight: str
    ) -> Optional[tuple]:
    ...
    async def _dijkstra_with_excluded_edges(
        self, graph: nx.DiGraph, start: str, end: str, weight: str, excluded: set
    ) -> Optional[tuple]:
    ...
    def _calculate_path_cost(
        self, graph: nx.DiGraph, path: list[str], weight: str
    ) -> float:
